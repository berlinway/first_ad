...
def build_model(config, input_dim = 3):
    arch = config['model']['arch']   #ours
    in_channel = config['model']['input_channel']  #1
    feat_dim = config['model']['feature_dim']  #1024
    n_label = config['model']['n_label'] #3
    n_hid_main = config['model']['nhid'] #512
    out_dim = config['adv_model']['out_dim'] #12
    expansion = config['model']['expansion'] #8
    type_name = config['model']['type_name'] #conv3x3x3
    norm_type = config['model']['norm_type']  #Instance
